...
function addServicesToCheckOut() {
    ...
    if (total > 0)
        inputTotal.val(parseInt(total) + parseInt(orderBonusInput.val()));
    inputAPayer.val(total - parseInt(reductionAmountInput.val()));
}
...
function confirmOrderUpdate(id) {
    ...
    let completeCallback = () => {
        window.location.href='/store/read-orders';
    };
    if (cart.length > 0) {
        makeAjaxCall("/store/update-order-form", "POST", params, null, completeCallback);
    } else {
        Swal.fire({
...
function confirmOrderUpdateAdmin(id) {
    let params = {
        order: id,
        rows: cart,
        total: (total + parseInt(orderBonusInput.val())),
    ...
    if (cart.length > 0) {
        makeAjaxCall("/Orders/update-order-form-admin", "POST", params, null, completeCallback);
    } else {
        Swal.fire({
            text: "Veuillez ajouter au moins un service au panier pour passer la commande",
            title: 'Success',
